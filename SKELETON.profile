...
/**
 * @file
 * Enables modules and site configuration for a ***MACHINE_NAME*** site installation.
 */

/*
 * Define ***MACHINE_NAME*** minimum execution time required to operate.
 */
...
function ***MACHINE_NAME***_install_tasks_alter(&$tasks, $install_state) {
  ...
  $tasks['install_finished']['function'] = '***MACHINE_NAME***_install_finished';
}
...
function ***MACHINE_NAME***_form_install_configure_form_alter(&$form, $form_state) {
  ...
  ***MACHINE_NAME***_clear_messages();
  ...
  $form['#submit'][] = '***MACHINE_NAME***_admin_save_fullname';
  $form['#submit'][] = '***MACHINE_NAME***_check_acquia_connector';
}
...
function ***MACHINE_NAME***_update_projects_alter(&$projects) {
  // Enable update status for the ***MACHINE_NAME*** profile.
  $modules = system_rebuild_module_data();
  // The module object is shared in the request, so we need to clone it here.
  $***MACHINE_NAME*** = clone $modules['commons'];
  $***MACHINE_NAME***->info['hidden'] = FALSE;
  _update_process_info_list($projects, array('***MACHINE_NAME***' => $commons), 'module', TRUE);
}
 ...
 */
function ***MACHINE_NAME***_install_tasks() {
  ...
  $demo_content = variable_get('***MACHINE_NAME***_install_example_content', FALSE);
  $acquia_connector = variable_get('***MACHINE_NAME***_install_acquia_connector', FALSE);

  return array(
    '***MACHINE_NAME***_acquia_connector_enable' => array(
      'display' => FALSE,
      'type' => '',
      'run' => $acquia_connector ? INSTALL_TASK_RUN_IF_NOT_COMPLETED : INSTALL_TASK_SKIP,
    ),
    '***MACHINE_NAME***_installer_palette' => array(
      'display_name' => st('Choose site color palette'),
      'display' => TRUE,
      'type' => 'form',
      'function' => '***MACHINE_NAME***_installer_palette',
    ),
    '***MACHINE_NAME***_anonymous_message_homepage' => array(
      'display_name' => st('Enter Homepage welcome text'),
      'display' => TRUE,
      'type' => 'form',
      'function' => '***MACHINE_NAME***_anonymous_welcome_text_form'
    ),
    '***MACHINE_NAME***_revert_features' => array(
      'display' => FALSE,
    ),
    '***MACHINE_NAME***_demo_content' => array(
      'display' => FALSE,
      'type' => '',
      'run' => $demo_content ? INSTALL_TASK_RUN_IF_NOT_COMPLETED : INSTALL_TASK_SKIP,
    ),
    '***MACHINE_NAME***_create_first_group' => array(
      'display_name' => st('Create the first group'),
      'display' => TRUE,
      'type' => 'form',
    ),
    '***MACHINE_NAME***_admin_permissions' => array(
      'display' => FALSE,
    ),
  );
}
 ...
 * the ***MACHINE_NAME*** installation.
 */
function ***MACHINE_NAME***_installer_palette() {
  $form = array();
  require_once(drupal_get_path('theme', '***MACHINE_NAME***_origins') . '/commons_origins.palettes.inc');

  ***MACHINE_NAME***_origins_palettes_form($form);
  $form['***MACHINE_NAME***_origins_palette_fieldset']['#collapsible'] = FALSE;
  $form['***MACHINE_NAME***_origins_palette_fieldset']['#collapsed'] = FALSE;
  $form['submit'] = array(
    '#type' => 'submit',
    '#value' => st('Save color palette'),
  );
  drupal_add_css('profiles/***MACHINE_NAME***/commons_installer.css');
 ...
 */
function ***MACHINE_NAME***_create_first_group() {
  $form['***MACHINE_NAME***_first_group_explanation'] = array(
    '#markup' => '<h2>' . st('Create the first group in your new community.') . '</h2>' . st("***MACHINE_NAME*** uses groups to collect community members and content related to a particular interest, working goal or geographic area."),
    '#weight' => -1,
  );

  $form['***MACHINE_NAME***_fist_group_example'] = array(
    '#markup' => theme('image', array('path' => 'profiles/***MACHINE_NAME***/images/commons_group_description_sample.png', 'alt' => 'Group description page example', 'alt' => 'Group description example')),
    '#weight' => 0,
  );

  $form['***MACHINE_NAME***_first_group_title'] = array(
    '#type' => 'textfield',
    '#title' => st("Group name"),
  ...
  $form['***MACHINE_NAME***_first_group_body'] = array(
    '#type' => 'textarea',
  ...
  $form['***MACHINE_NAME***_first_group_submit'] = array(
    '#type'  => 'submit',
 ...
 *
 * @see ***MACHINE_NAME***_create_first_group().
 */
function ***MACHINE_NAME***_create_first_group_submit($form_id, &$form_state) {
  ...
  $first_group->title = $values['***MACHINE_NAME***_first_group_title'];
  $first_group->body[LANGUAGE_NONE][0]['value'] = $values['***MACHINE_NAME***_first_group_body'];
  $first_group->uid = 1;
 ...
 */
function ***MACHINE_NAME***_revert_features() {
  // Revert Features components to ensure that they are in their default states.
  $revert = array(
    '***MACHINE_NAME***_groups' => array('field_instance'),
    '***MACHINE_NAME***_wikis' => array('og_features_permission'),
    '***MACHINE_NAME***_wysiwyg' => array('user_permission', 'ckeditor_profile'),
  );
  features_revert($revert);
}

function ***MACHINE_NAME***_admin_permissions() {
 ...
 */
function ***MACHINE_NAME***_admin_save_fullname($form_id, &$form_state) {
  $values = $form_state['values'];
 ...
 */
function ***MACHINE_NAME***_check_acquia_connector($form_id, &$form_state) {
  $values = $form_state['values'];
  if (isset($values['enable_acquia_connector']) && $values['enable_acquia_connector'] == 1) {
    ...
    variable_set('***MACHINE_NAME***_install_acquia_connector', TRUE);
    variable_set('***MACHINE_NAME***_install_acquia_modules', array_keys($options));
  }
}
 ...
 */
function ***MACHINE_NAME***_anonymous_welcome_text_form() {
  $form['***MACHINE_NAME***_anonymous_welcome_explanation'] = array(
  ...
  $form['***MACHINE_NAME***_anonymous_welcome_example'] = array(
    '#markup' => theme('image', array('path' => 'profiles/***MACHINE_NAME***/images/commons_homepage_text_example.png', 'alt' => 'Home page example', 'alt' => 'Home page example')),
    '#weight' => 0,
  );

  $form['***MACHINE_NAME***_anonymous_welcome_title'] = array(
    '#type' => 'textfield',
  ...
  $form['***MACHINE_NAME***_anonymous_welcome_body'] = array(
    '#type' => 'textarea',
  ...
  $form['***MACHINE_NAME***_install_example_content'] = array(
    '#type' => 'checkbox',
    '#title' => st('Install example content'),
    '#description' => st('Install ***MACHINE_NAME*** with example content so that you can get a sense of what your site will look like once it becomes more active. Example content includes a group, a few users and content for that group. Example content can be modified or deleted like normal content.'),
    '#default_value' => TRUE
  );

  $form['***MACHINE_NAME***_anonymous_welcome_submit'] = array(
    '#type'  => 'submit',
    '#value' => st('Save and continue')
  );
 ...
 * @see ***MACHINE_NAME***_anonymous_welcome_text_form()
 */
function ***MACHINE_NAME***_anonymous_welcome_text_form_submit($form_id, &$form_state) {
  variable_set('***MACHINE_NAME***_anonymous_welcome_title', $form_state['values']['commons_anonymous_welcome_title']);
  variable_set('***MACHINE_NAME***_anonymous_welcome_body', $form_state['values']['commons_anonymous_welcome_body']);
  variable_set('***MACHINE_NAME***_install_example_content', $form_state['values']['commons_install_example_content']);
}

/**
 * Helper function to generate a machine name similar to the user's full name.
 */
function ***MACHINE_NAME***_normalize_name($name) {
  return drupal_strtolower(str_replace(' ','_', $name));
}
/**
 * This function generate a demo content
 */
function ***MACHINE_NAME***_demo_content() {
  ...
  foreach ($demo_users as $name) {
    ...
    $normalize_name = ***MACHINE_NAME***_normalize_name($name);
    ...
    ***MACHINE_NAME***_add_user_avatar($demo_users[$name]);
  }
  ...
  $boston_group->body[LANGUAGE_NONE][0]['value'] = ***MACHINE_NAME***_veggie_ipsum();
  ...
  $nyc_group->body[LANGUAGE_NONE][0]['value'] = ***MACHINE_NAME***_veggie_ipsum();
...
<li>City Girl Cafe for the ambiance <li>Bom Cafe for granola";
  ...
  foreach ($terms as $term) {
    $post->field_topics[LANGUAGE_NONE][]['tid'] = ***MACHINE_NAME***_create_topic($term);
  }
  ...
  foreach ($terms as $term) {
    $wiki->field_topics[LANGUAGE_NONE][]['tid'] = ***MACHINE_NAME***_create_topic($term);
  }
  ...
  foreach ($terms as $term) {
    $event->field_topics[LANGUAGE_NONE][]['tid'] = ***MACHINE_NAME***_create_topic($term);
  }
  ...
  ***MACHINE_NAME***_clear_messages();
  ...
  variable_del('***MACHINE_NAME***_install_example_content');
...
function ***MACHINE_NAME***_add_user_avatar($account) {
  global $base_url;

  if ($account->uid) {
    $picture_directory =  file_default_scheme() . '://' . variable_get('user_picture_path', 'pictures');
    if(file_prepare_directory($picture_directory, FILE_CREATE_DIRECTORY)){
      $picture_result = drupal_http_request($base_url . '/profiles/***MACHINE_NAME***/images/avatars/avatar-' . commons_normalize_name($account->name) . '.png');
 ...
 */
function ***MACHINE_NAME***_veggie_ipsum() {
 ...
 * for a new instalations of Drupal ***MACHINE_NAME***
 *
 * @see ***MACHINE_NAME***_demo_content().
 */
function ***MACHINE_NAME***_create_topic($topic_name = '') {
  $term = new stdClass();
 ...
 */
function ***MACHINE_NAME***_acquia_connector_enable() {
  $modules = variable_get('***MACHINE_NAME***_install_acquia_modules', array());
  if (!empty($modules)) {
 ...
 */
function ***MACHINE_NAME***_install_finished(&$install_state) {
  ...
  $footer_body = '<p>'. st('A ***MACHINE_NAME*** Community, powered by <a href="@acquia">Acquia</a>', array('@acquia' => url('https://www.acquia.com/products-services/drupal-commons-social-business-software'))) . '</p>';
  ...
  if (drupal_write_record('block_custom', $footer_block_text)) {
    $footer_block = array(
      ...
      'theme' => '***MACHINE_NAME***_origins',
      'visibility' => 0,
      'region' => 'footer',
      'status' => 1,
      'pages' => 0,
      'weight' => 1,
      'title' => variable_get('site_name', 'Drupal ***MACHINE_NAME***'),
    );
 ...
 */
function ***MACHINE_NAME***_clear_messages() {
 ...
 */
function ***MACHINE_NAME***_set_default_avatar() {
  global $base_url;
  $picture_directory =  file_default_scheme() . '://' . variable_get('user_picture_path', 'pictures');
  if(file_prepare_directory($picture_directory, FILE_CREATE_DIRECTORY)){
    $picture_result = drupal_http_request($base_url . '/profiles/***MACHINE_NAME***/images/avatars/user-avatar.png');
